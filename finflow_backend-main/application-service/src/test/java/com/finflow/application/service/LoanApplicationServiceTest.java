...
class LoanApplicationServiceTest {
    ...
    void updateStatus_AdminShouldBeBoundByStateMachine() {
        ...
        assertTrue(exception.getMessage().contains("Decision Blocked"));
        verify(repository, never()).save(any());
    }
    ...
    void isValidTransition_FromFinalStates() {
        ...
        // APPROVED and REJECTED are terminal (cannot transition to anything)
    ...
    void submitApplicantApplication_ShouldRequireDocumentsCompleted() {
        ...
        draftApp.setApplicantId("applicant-1");
        draftApp.setApplicantUsername("hello");
        when(repository.findByApplicantIdOrderByCreatedAtDesc("applicant-1"))
                .thenReturn(java.util.Collections.singletonList(draftApp));
        when(repository.save(any(LoanApplication.class))).thenReturn(draftApp);
    ...
    void getApplicationsByUser_ShouldReturnList() {
        when(repository.findByApplicantUsernameOrderByCreatedAtDesc("hello")).thenReturn(java.util.Collections.singletonList(draftApp));
