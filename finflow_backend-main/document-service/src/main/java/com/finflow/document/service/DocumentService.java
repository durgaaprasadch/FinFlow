    ...
    public List<LoanDocument> uploadAllRequiredDocuments(
        ...
        try {
            if (aadhaarFile != null && !aadhaarFile.isEmpty()) 
                uploaded.add(storeDocument(aadhaarFile, applicationId, com.finflow.document.entity.DocumentType.AADHAAR, userId));
            if (panFile != null && !panFile.isEmpty()) 
                uploaded.add(storeDocument(panFile, applicationId, com.finflow.document.entity.DocumentType.PAN, userId));
            if (salarySlipFile != null && !salarySlipFile.isEmpty()) 
                uploaded.add(storeDocument(salarySlipFile, applicationId, com.finflow.document.entity.DocumentType.SALARY_SLIP, userId));
            if (bankStatementFile != null && !bankStatementFile.isEmpty()) 
                uploaded.add(storeDocument(bankStatementFile, applicationId, com.finflow.document.entity.DocumentType.BANK_STATEMENT, userId));
            if (photoFile != null && !photoFile.isEmpty()) 
                uploaded.add(storeDocument(photoFile, applicationId, com.finflow.document.entity.DocumentType.PHOTO, userId));
    ...
    public byte[] generateZipOfDocuments(Long applicationId) {
        ...
        try (java.util.zip.ZipOutputStream zos = new java.util.zip.ZipOutputStream(baos)) {
            ...
            for (LoanDocument doc : latestDocs) {
                if (doc == null || doc.getDocumentData() == null) continue;
