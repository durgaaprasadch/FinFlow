...
public class ApplicantApplicationController {
    ...
    public ApiResponse<Map<String, Object>> createApplication(
            @RequestBody CreateApplicationRequest createRequest,
    ...
    public ApiResponse<Map<String, Object>> updateLoanDetails(
            @RequestBody LoanDetailsRequest loanRequest,
            HttpServletRequest request) {
        String role = getHeader(request, HEADER_USER_ROLE, ROLE_APPLICANT);
        String username = request.getHeader(HEADER_LOGGED_IN_USER);
        String id = request.getHeader(HEADER_APPLICANT_ID);
    ...
    public ApiResponse<Map<String, Object>> getApplicationStatus(HttpServletRequest request) {
        ...
        return service.findActiveApplicationByApplicant(id, username)
                .map(app -> {
                    Map<String, Object> data = toSummary(app);
                    data.put("timeline", service.getApplicantTimeline(app.getId(), id, username));
                    
                    // Add all fields needed by frontend for state repopulation
                    data.put("tenureMonths", app.getTenure());
                    data.put("purpose", app.getPurpose());
                    data.put("fullName", app.getFullName());
                    data.put("dob", app.getDob() != null ? app.getDob().toString() : "");
                    data.put("gender", app.getGender());
                    data.put("maritalStatus", app.getMaritalStatus());
                    data.put("panNumber", app.getPanNumber());
                    data.put("aadhaarNumber", app.getAadhaarNumber());
                    data.put("line1", app.getAddressLine1());
                    data.put("city", app.getCity());
                    data.put("state", app.getState());
                    data.put("pincode", app.getPincode());
                    data.put("employmentType", app.getEmploymentType());
                    data.put("companyName", app.getCompanyName());
                    data.put("designation", app.getDesignation());
                    data.put("monthlyIncome", app.getMonthlyIncome());
                    data.put("experienceYears", app.getExperienceYears());
                    data.put("reuploadModules", app.getReuploadModules());
                    
                    return ApiResponse.success("Current application status fetched", data);
                })
                .orElseGet(() -> ApiResponse.success("No active application found", null));
    }
    ...
    @Tag(name = "Loan Applicant Journey")
    @Operation(summary = "Action: Delete specific application", description = "Permanently remove an application by its ID.")
    @DeleteMapping("/{id}")
    public ApiResponse<String> deleteApplication(@PathVariable Long id, HttpServletRequest request) {
        String role = getHeader(request, HEADER_USER_ROLE, ROLE_APPLICANT);
        String username = request.getHeader(HEADER_LOGGED_IN_USER);
        String applicantId = request.getHeader(HEADER_APPLICANT_ID);
        ensureApplicantRole(role);
        
        service.deleteApplicationById(id, applicantId, username);
        return ApiResponse.success("Application deleted successfully", "DELETED");
    }
    ...
    @GetMapping("/{id}")
    public LoanApplicationResponse getById(@PathVariable("id") Long id) {
    ...
    public List<LoanApplicationResponse> getAllApplications(
            @RequestHeader(value = "userRole", defaultValue = "APPLICANT") String role) {
        if (!ROLE_ADMIN.equalsIgnoreCase(role)) {
            throw new com.finflow.application.exception.ApplicationException("Unauthorized: Only ADMIN can view all applications");
        }
    ...
    @io.swagger.v3.oas.annotations.Hidden
    @GetMapping("/admin-review/{applicantId:.+}")
    public LoanApplicationResponse getByApplicant(
            @PathVariable("applicantId") String applicantId,
            @RequestHeader(value = "userRole", defaultValue = "APPLICANT") String role) {
        if (!ROLE_ADMIN.equalsIgnoreCase(role)) {
            throw new com.finflow.application.exception.ApplicationException("Unauthorized: Only ADMIN can view applications by applicant ID");
        }
        java.util.Optional<LoanApplication> app;
        if (applicantId.matches("^\\d+$")) {
            app = java.util.Optional.ofNullable(service.getApplicationById(Long.valueOf(applicantId)));
        } else {
            app = service.findActiveApplicationByApplicant(applicantId, applicantId);
        }
        return LoanApplicationResponse.fromEntity(app.orElseThrow(() -> 
            new com.finflow.application.exception.ApplicationException("No application found for applicant: " + applicantId)));
    }

    @Operation(summary = "Internal: Update application status")
    @PatchMapping("/{id}/status")
    public LoanApplicationResponse updateStatusInternal(
            @PathVariable("id") Long id,
            @RequestParam("status") com.finflow.application.entity.LoanStatus status,
            @RequestParam(value = "remarks", required = false) String remarks,
            @RequestParam(value = "reuploadModules", required = false) String reuploadModules,
            @RequestHeader(value = "userRole", defaultValue = "APPLICANT") String role,
            @RequestHeader(value = "loggedInUser", required = false) String loggedInUser) {
        return LoanApplicationResponse.fromEntity(service.updateStatus(id, status, role, loggedInUser, remarks, reuploadModules));
    }

    @io.swagger.v3.oas.annotations.Hidden
    @GetMapping("/admin-review/{applicantId:.+}/history")
    public List<Map<String, Object>> getHistoryByApplicant(
            @PathVariable("applicantId") String applicantId,
            @RequestHeader(value = "userRole", defaultValue = "APPLICANT") String role) {
        if (!ROLE_ADMIN.equalsIgnoreCase(role)) {
            throw new com.finflow.application.exception.ApplicationException("Unauthorized: Only ADMIN can view application history");
        }
        java.util.Optional<LoanApplication> app;
        if (applicantId.matches("^\\d+$")) {
            app = java.util.Optional.ofNullable(service.getApplicationById(Long.valueOf(applicantId)));
        ...
        LoanApplication loanApp = app.orElseThrow(() -> 
            new com.finflow.application.exception.ApplicationException("No application found for applicant: " + applicantId));
        return service.getApplicantTimeline(loanApp.getId(), loanApp.getApplicantId(), loanApp.getApplicantUsername());
    }

    private Map<String, Object> toSummary(LoanApplication app) {
        Map<String, Object> summary = new LinkedHashMap<>();
        summary.put(FIELD_APPLICATION_ID, app.getId());
        summary.put("applicantUsername", app.getApplicantUsername());
        ...
        summary.put("reuploadModules", app.getReuploadModules());
