    ...
    public LoanApplication updateStatus(Long id, LoanStatus newStatus, String role, String loggedInUser, String remarks,
        ...
        if (!isValidTransition(currentStatus, newStatus)) {
            throw new InvalidTransitionException(
                    "Decision Blocked: Strict transitions policy violated. Cannot transition from " + currentStatus
                            + " to " + newStatus);
        }
    ...
    public void syncStatusAfterDocumentUpload(Long applicationId, String documentType) {
        ...
        if (current == LoanStatus.REUPLOAD) {
            ...
            logStatusChange(app.getId(), current, LoanStatus.DOCS_REUPLOADED, "SYSTEM",
                    "Automated sync for " + documentType);
        } else if (current == LoanStatus.PARTIAL || current == LoanStatus.DOCS_PENDING || current == LoanStatus.DRAFT) {
            app.setStatus(LoanStatus.DOCUMENTS_COMPLETED);
            logStatusChange(app.getId(), current, LoanStatus.DOCUMENTS_COMPLETED, "SYSTEM",
                    "Automated completion sync for " + documentType);
        }
        ...
        try {
            ...
            for (Object obj : uploadedDocs) {
                if (obj instanceof Map<?, ?> map) {
                    ...
                    if (type != null)
                        uploadedDocumentTypes.add(type.toString().toUpperCase());
                }
            }
            log.info("[SYNC-DEBUG] App {}: Required: {}. Found: {}", application.getId(), requiredDocumentTypes,
    ...
    private boolean isValidTransition(LoanStatus current, LoanStatus next) {
        return switch (current) {
            ...
            case DOCS_REUPLOADED -> next == LoanStatus.APPROVED || next == LoanStatus.REJECTED
                    || next == LoanStatus.REUPLOAD || next == LoanStatus.REVIEW || next == LoanStatus.SUBMITTED
                    || next == LoanStatus.DOCS_REUPLOADED;
