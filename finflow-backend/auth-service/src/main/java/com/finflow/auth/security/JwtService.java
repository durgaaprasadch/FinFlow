...
public class JwtService {
    ...
    public String generateToken(String email, String role, java.util.UUID userId) {
        ...
        // Standardize role with ROLE_ prefix if not present
        String effectiveRole = role != null && !role.startsWith("ROLE_") ? "ROLE_" + role.toUpperCase() : role;
    ...
    public String extractUserId(String token) {
        return (String) Jwts.parserBuilder().setSigningKey(getSignKey()).build().parseClaimsJws(token).getBody().get("userId");
    }
