...
public class DocumentService {
    ...
    public DocumentService(LoanDocumentRepository repository, DocumentEventPublisher eventPublisher,
    ...
    public LoanDocument uploadDocument(MultipartFile file, Long applicationId,
            com.finflow.document.entity.DocumentType documentType, String userId, String role) {
        try {
    ...
    public List<LoanDocument> uploadAllRequiredDocuments(
        ...
        List<LoanDocument> uploaded = new java.util.ArrayList<>();
        try {
            if (aadhaarFile != null && !aadhaarFile.isEmpty())
                uploaded.add(storeDocument(aadhaarFile, applicationId, com.finflow.document.entity.DocumentType.AADHAAR,
                        userId));
            if (panFile != null && !panFile.isEmpty())
                uploaded.add(
                        storeDocument(panFile, applicationId, com.finflow.document.entity.DocumentType.PAN, userId));
            if (salarySlipFile != null && !salarySlipFile.isEmpty())
                uploaded.add(storeDocument(salarySlipFile, applicationId,
                        com.finflow.document.entity.DocumentType.SALARY_SLIP, userId));
            if (bankStatementFile != null && !bankStatementFile.isEmpty())
                uploaded.add(storeDocument(bankStatementFile, applicationId,
                        com.finflow.document.entity.DocumentType.BANK_STATEMENT, userId));
            if (photoFile != null && !photoFile.isEmpty())
                uploaded.add(storeDocument(photoFile, applicationId, com.finflow.document.entity.DocumentType.PHOTO,
                        userId));
    ...
    private void validateUploadAccess(Long applicationId, String userId, String role) {
        ...
        boolean matchesApplicantUsername = applicantUsername != null
                && userId.equalsIgnoreCase(applicantUsername.toString());
    ...
    public List<LoanDocument> getDocumentsForApplication(Long applicationId) {
        ...
        // Group by type and pick latest version of each, filtering out any null types
    ...
    public byte[] generateZipOfDocuments(Long applicationId) {
        ...
        try (java.util.zip.ZipOutputStream zos = new java.util.zip.ZipOutputStream(baos)) {
            ...
            for (LoanDocument doc : latestDocs) {
                ...
                String typeName = doc.getDocumentType() != null ? doc.getDocumentType().name().toLowerCase()
                        : "document_" + doc.getDocumentId();
    ...
    public Optional<LoanDocument> getDocumentByAppAndType(Long applicationId,
            com.finflow.document.entity.DocumentType type) {
    ...
    private LoanDocument storeDocument(MultipartFile file, Long applicationId,
            com.finflow.document.entity.DocumentType documentType, String userId) throws IOException {
